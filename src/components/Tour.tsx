...
const Tour = (props: any) => {
  return (
    <main {...storyblokEditable(props.blok)} className="tour-page-main container mx-auto px-4 w-full pt-16 pb-16">
      ...
      <img className='mt-12 rounded-lg' src={props.blok.main_image.filename} alt={props.blok.main_image?.alt || props.blok.name} />
      <p className='mt-12 text-lg md:text-2xl md:leading-relaxed'>
        {props.blok.introduction}
      </p>
      <div className='tour-body-main mt-12'>
        <div className='image-container'>
          <img className='img-secondary rounded-lg' src={props.blok.secondary_image.filename} alt={props.blok.secondary_image?.alt || props.blok.name}/>
        </div>
